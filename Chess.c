...
#include <stdlib.h>   // standard lib for rand and memory
#include <stdbool.h>  // alows using bool for true/false
#include <time.h>     // time functions for random seed
#include <math.h>     // math labrary for abs function
...
void clearBoard() {
    for (int i = 0; i < boardSize; i++) 
        for (int j = 0; j < boardSize; j++) 
            board[i][j] = ' '; // puts a blank space everywhere
}
...
bool isRookPathClear(int fromR, int fromC, int toR, int toC) {
    if (fromR != toR && fromC != toC) return false; // rook cant move diagnal
    
    int stepR = (toR > fromR) ? 1 : (toR < fromR ? -1 : 0); // row direction step multiplier
    int stepC = (toC > fromC) ? 1 : (toC < fromC ? -1 : 0); // col direction step multiplier
    
    int currR = fromR + stepR; // advances past starting row square
    int currC = fromC + stepC; // advances past starting col square
    
    while (currR != toR || currC != toC) { // loops path until destination
        if (board[currR][currC] != ' ') return false; // path blocked by a piece
        currR += stepR; 
        currC += stepC; 
    }
    return true;      // path is completely empty
}

bool isSquareAttackedByPlayer(int r, int c) {
    if (abs(r - playerKing.r) <= 1 && abs(c - playerKing.c) <= 1) return true; // targeted by king
    if (rook1Alive && isRookPathClear(rook1.r, rook1.c, r, c)) return true; // targeted by first rook
    if (rook2Alive && isRookPathClear(rook2.r, rook2.c, r, c)) return true; // targeted by second rook
    return false;     
}
    ...
    do {
        ...
        do {
            ...
            playerKing.c = rand() % boardSize; 
                 ...
                 (abs(playerKing.r - compKing.r) <= 1 && abs(playerKing.c - compKing.c) <= 1)); // no overlapping or illegal adjacent kings
        ...
        rook2Alive = true; 
    } while (isSquareAttackedByPlayer(compKing.r, compKing.c)); // restarts setup if computer starts in check
}

bool computerMakeMove() {
    int dr[] = {-1, -1, -1,  0, 0,  1, 1, 1}; // row directional adjustments offset
    int dc[] = {-1,  0,  1, -1, 1, -1, 0,  1}; // col directional adjustments offset
    Pos validMoves[8]; 
    bool isCaptureMove[8]; 
    int count = 0;    

    for (int i = 0; i < 8; i++) { // loops through all 8 square choices
        ...
        if (inBounds(nr, nc)) { 
            if (board[nr][nc] == ' ' || board[nr][nc] == 'R' || board[nr][nc] == 'W') { 
                ...
                if (backup == 'R') rook1Alive = false; // temp kills rook for simulation
                if (backup == 'W') rook2Alive = false; // temp kills rook for simulation

                board[compKing.r][compKing.c] = ' '; board[nr][nc] = 'k'; 
                compKing.r = nr; compKing.c = nc; 

                if (!isSquareAttackedByPlayer(nr, nc) && !(abs(nr - playerKing.r) <= 1 && abs(nc - playerKing.c) <= 1)) { // is square safe
                    validMoves[count] = compKing; 
                    isCaptureMove[count] = (backup == 'R' || backup == 'W'); // logs if move captures rook
                    count++;  
                }
    ...
    if (count > 0) {  
        int chosenIndex = 0; 
        for (int i = 0; i < count; i++) { if (isCaptureMove[i]) { chosenIndex = i; break; } } // prioritizes capturing player rooks
    ...
    return false;     // no safe moves left checkmate or stalemate
}
...
void initLog(unsigned int seed) {
    ...
    gameLog.count = 0;   
    gameLog.capacity = 16; 
    gameLog.moves = malloc(gameLog.capacity * sizeof(Move)); // allocates start tracking memory
}

void logMove(char piece, int fromR, int fromC, int toR, int toC, bool gaveCheck) {
    if (gameLog.count == gameLog.capacity) { 
        gameLog.capacity *= 2; 
        gameLog.moves = realloc(gameLog.moves, gameLog.capacity * sizeof(Move)); // expands dynamic array buffer
    }
...
void freeLog() {
    if (gameLog.moves != NULL) { 
        free(gameLog.moves); // releases allocated log storage space
        gameLog.moves = NULL; 
    }
    gameLog.count = 0;   
    gameLog.capacity = 0; 
...
void coordToStr(int r, int c, int bSize, char* out) {
    out[0] = 'A' + c; // translates numeric col to uppercase letter
    sprintf(out + 1, "%d", bSize - r); // converts row number indexing string format
}
...
void saveGame(GameLog* log, const char* filename) {
    FILE* f = fopen(filename, "w"); // opens text doc write stream
...
void replayGame(const char* filename) {
    ...
    srand(log.seed);  
    generatePositions(); // reproduces identical starting layout position map
    
    printf("\n--- REPLAY (Press Enter for next move) ---\n"); 
    printBoard();     

    int ch;           
    while ((ch = getchar()) != '\n' && ch != EOF); // clears user entry stream buffer cache

    for (int i = 0; i < log.count; i++) { 
        printf("\n[Press Enter to see move %d/%d...]", i + 1, log.count); 
        while (getchar() != '\n'); // halts script progression until Enter pressed
...
void startNewGame() {
    unsigned int seed = (unsigned int)time(NULL); // gets current unix timestamp reference
    srand(seed);      
    generatePositions(); 
    initLog(seed);    
    printf("\n--- GAME STARTED ---\n"); 
    while (1) {       
        ...
        int fromC = col1 - (col1 >= 'a' ? 'a' : 'A'); int fromR = boardSize - row1; // index mapping translations column row
        int toC = col2 - (col2 >= 'a' ? 'a' : 'A'); int toR = boardSize - row2; // index mapping translations target column row
        ...
        if (piece == 'K') { 
            if (abs(toR - fromR) <= 1 && abs(toC - fromC) <= 1) { 
                if (abs(toR - compKing.r) > 1 || abs(toC - compKing.c) > 1) validMove = true; // king isolation spacing verification check
        ...
        if (!rook1Alive && !rook2Alive) { // validation draw checker logic parameters trigger condition
            printBoard(); 
...
int main() {
    ...
    while (1) {       
        ...
        if (scanf("%d", &choice) != 1) { while (getchar() != '\n'); continue; } // catches and flushes alphabetical entry terminal stream crashes loop back
        if (choice == 1) startNewGame(); 
        else if (choice == 2) changeBoardSize(); 
        else if (choice == 3) replayGame("save.txt"); 
        else if (choice == 4) return 0; // safely breaks infinite wrapper loops killing execution thread paths exiting back to OS console environments
    }
    return 0;         
}